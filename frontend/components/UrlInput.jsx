...
export default function UrlInput({ setTranscript }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-4 w-full max-w-3xl mx-auto mt-6">
      <input
        type="text"
      ...
      />
      <button
        className="ml-5 px-6 py-3 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all disabled:bg-indigo-300 disabled:cursor-not-allowed"
        onClick={handleFetch}
        disabled={loading}
      >
      ...
      {/* Error message box */}
      {errorMessage && (
        <div className="mt-4 w-full text-left bg-red-100 border border-red-400 text-red-800 px-4 py-3 rounded-lg">
          {errorMessage}
        </div>
      )}
    </div>
